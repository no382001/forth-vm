...
auto load_from_stdin(vm &v, uint16_t addr = 0) -> size_t {
  auto bytes = std::vector<uint8_t>{std::istreambuf_iterator<char>{std::cin},
                                    std::istreambuf_iterator<char>{}};
  std::copy(bytes.begin(), bytes.end(), v.mem.begin() + addr);
...
auto main(int argc, char **argv) -> int {
  static struct option long_options[] = {{"trace", no_argument, nullptr, 't'},
                                         {"help", no_argument, nullptr, 'h'},
                                         {nullptr, 0, nullptr, 0}};
  auto v = vm{};
  v.trap_ext = chip8_trap_handler;
